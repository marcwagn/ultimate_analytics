...
def convert_video_annotations(source: Union[str, dict]) -> pd.DataFrame:
    ...
    if "key" in objects_list[0]:
        ...
        raise ValueError(
            "The JSON annotations file is expected to have either objects[...].key identifier, or objects[...].id"
        )

    class_to_idx_map = {
        m[objects_class_id_key_name]: i for i, m in enumerate(objects_list)
    }
...
def convert_single_image_annotation_file_to_pose_estimation(
    ...
    for detected in annotations["objects"]:
        ...
        for i, node_key in enumerate(graph_id_to_nodes[class_id]):
            if node_key in detected_nodes:
                ...
                scaled_x, scaled_y = scale_point(
                    x=detected_nodes[node_key]["loc"][0],
                    y=detected_nodes[node_key]["loc"][1],
                )
        ...
        box_scaled_arr = _xyxy2xywhn(
            np.array([min_x, min_y, max_x, max_y], dtype="float"), h=height, w=width
        )

        vector = np.concatenate(
            (np.array([class_idx]), box_scaled_arr, output_coords_flattened), axis=0
        )
...
def _source_to_annotation_generators(
    source: Union[str, dict[str, JSONGenerator]],
) -> dict[str, JSONGenerator]:
    """
    ...
    if source is None:
        ...
        if not os.path.isdir(source) or not os.path.exists(source):
            raise ValueError(
                "If source is passed as string, it needs to represent an existing directory"
            )
        annotations_generators = {
            file: (lambda: _read_json_content(file, root=source))
            for file in list_files_only(source)
        }
    elif isinstance(source, dict):
        if not all([isinstance(o, Callable) for o in source.values()]):
            raise ValueError(
                "If source is passed as a dict, it needs to be a dict (keyed by file name) of callables returning JSON dicts"
            )
